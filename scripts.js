...
document.addEventListener('DOMContentLoaded', function() {
    const journalStartDate = new Date('2022-11-28');
    const today = new Date();
    const timeDifference = today - journalStartDate;
    const daysSinceStart = Math.floor(timeDifference / (1000 * 60 * 60 * 24));

    const journalDaysElement = document.getElementById('journal-days');
    if (journalDaysElement) {
        journalDaysElement.textContent = daysSinceStart;
    }
});
